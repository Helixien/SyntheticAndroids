...
{
	public class JobDriver_AndroidIngest : JobDriver
	{
		...
		public override IEnumerable<Toil> MakeNewToils()
		{
			...
			yield return Toils_Jump.JumpIf(chew, () => job.GetTarget(TargetIndex.A).Thing is Corpse && pawn.needs.TryGetNeed<Need_Energy>().CurLevelPercentage < 0.9f);
		}
		...
		public static Toil FinalizeIngest(Pawn ingester, TargetIndex ingestibleInd)
		{
			...
			toil.initAction = delegate
			{
				...
				var need_energy = ingester.needs.TryGetNeed<Need_Energy>();
				float num = need_energy.EnergyWanted;
				if (curJob.overeat)
				{
				...
				if (!ingester.Dead)
				{
					need_energy.CurLevel += num2;
				}
				ingester.needs.joy.CurLevel += num2 / 3f;
			};
